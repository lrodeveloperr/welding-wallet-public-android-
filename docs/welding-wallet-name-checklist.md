...
- Existing iOS bundle identifier values in project configuration
- Hosted policy URL path: `/privacy-policy/welding-gas-wallet/`

Changing an application/bundle identifier would create a different installed/store app and break in-place upgrades. The policy URL remains on the legacy slug so existing published links do not 404.
